...
const navItems = [
  ...
  { href: "/seja-luz", label: "Seja Luz", icon: Star },
  ...
  const sidebarContent = (
    <>
      ...
      <div className={cn(
        ...
        <div className="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center flex-shrink-0 animate-pulse-glow">
      ...
      <nav className="relative flex-1 py-4 px-2 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            <div key={item.href}>
              {showSeparator && <div className="mx-3 my-3 border-t border-zinc-800/60" />}
              {linkContent}
            </div>
          )
        })}
      ...
      <div className="relative hidden md:block border-t border-zinc-800/60 p-2"
        style={{ borderImage: "linear-gradient(90deg, transparent, rgba(245,158,11,0.15), transparent) 1" }}>
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 transition-all duration-200"
        >
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-zinc-950">
      ...
      <aside className={cn(
        ...
        <button onClick={() => setMobileOpen(false)} className="absolute top-4 right-3 p-1 rounded-md text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 transition-colors z-10">
          <X className="w-5 h-5" />
        </button>
        {sidebarContent}
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="relative h-16 border-b border-zinc-800/60 glass-strong flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <button className="md:hidden text-zinc-500 hover:text-zinc-300 p-1" onClick={() => setMobileOpen(true)}>
              <Menu className="w-5 h-5" />
            </button>
            <span className="text-xs text-zinc-600 font-mono">v1.0.0</span>
            <span className="text-zinc-800">|</span>
            <span className="text-xs text-zinc-500">{session?.user?.name || "Carregando..."}</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="text-zinc-500 hover:text-red-400 p-1" onClick={() => signOut({ callbackUrl: "/login" })}>
